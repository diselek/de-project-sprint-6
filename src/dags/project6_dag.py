...
@dag(schedule_interval='0 0 * * *', start_date=pendulum.parse('2023-02-12'),catchup=False)
def sprint6_project_dag_get_data():
    ...
    load_group_log = VerticaOperator(
        ...
        sql='''TRUNCATE TABLE brok01yandexru__STAGING.group_log;
        COPY brok01yandexru__STAGING.group_log(group_id, user_id, 
                                                    user_id_from, event, 
                                                    event_dt) 
        FROM LOCAL '/data/group_log.csv' DELIMITER ',';''')
